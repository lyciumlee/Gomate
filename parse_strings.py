...
def is_string_patt(addr):
    ...
    if idc.print_operand(addr, 0) in VALID_REGS \
        ...
        addr_2 = ida_search.find_code(addr, idaapi.SEARCH_DOWN)
        try:
        ...
        if idc.print_insn_mnem(addr_2) == 'mov' and dest_reg in VALID_DEST \
            ...
            addr_3 = ida_search.find_code(addr_2, idaapi.SEARCH_DOWN)
...
def parse_str_ptr(addr):
    ...
    str_len = idc.get_qword(str_len_addr)
    str_ptr = idc.get_qword(str_ptr_addr)
    str_addr = idc.get_qword(str_ptr)
    ...
    if str_len > 1:
        ...
        if ida_bytes.create_strlit(str_ptr, str_len, ida_nalt.STRTYPE_C):
            
            if opnd_diff > 0:
                idc.set_cmt(addr, "length: %d" % str_len, True)
                idaapi.add_dref(addr_2, str_ptr, idaapi.dr_O)
            else:
                idc.set_cmt(addr_2, "length: %d" % str_len, True)
...
def create_string(addr, string_len):
    ...
    if idc.get_strlit_contents(addr) is None and ida_bytes.create_strlit(addr, string_len, ida_nalt.STRTYPE_C):
        ...
        if ida_bytes.create_strlit(addr, string_len, ida_nalt.STRTYPE_C):
...
def parse_strings():
    ...
    #for addr in idautils.Functions(text_seg.startEA, text_seg.endEA):
            ...
            end_addr = next(idautils.Chunks(addr))[1]
            if(end_addr < addr):
